 ...
 * under the License.
 */
/*  Implementation of SWD protocol with Raspberry Pi Bidirectional SPI by Lup Yuen Lee
 * (luppy@appkaki.com) */
/*  Why implement SWD over Bidirectional SPI on Raspberry Pi?  Because SWD over Bit-Banging GPIO has
 * timing issues that affect OpenOCD flashing... */
/*  See
 * https://medium.com/@ly.lee/openocd-on-raspberry-pi-better-with-swd-on-spi-7dea9caeb590?source=friends_link&sk=df399bfd913d3e262447d28aa5af6b63 */
...
COMMAND_HANDLER(bcm2835spi_handle_swd_dir_gpionum_swdio);


/*  SPI Configuration */
static const char *device = "/dev/spidev0.0";	/*  SPI device name. If missing, enable SPI in
						 * raspi-config. */
static uint8_t mode = 0	/*  Note: SPI LSB mode is not supported on Broadcom. We must flip LSB to MSB
			 * ourselves. */
...
/*  31,200 kHz (31.2 MHz) for SPI speed. Use fastest speed
					 * possible because we resend JTAG-to-SWD sequence after
					 * every read */
static int speed_khz = 15000;  //  Slower: 1,953 kHz (1.9 MHz)
 /* static uint speed_khz = 122;   //  Slowest: 122 kHz */
static uint16_t delay = 0;		/*  SPI driver latency:
					 * https://www.raspberrypi.org/forums/viewtopic.php?f=44&t=19489 */

static int swdio_dir_gpio = -1;
static int swdio_dir_gpio_mode;

/*/ We need 2 transmit/receive buffers: One buffer in OpenOCD's LSB format, one buffer in Broadcom
 * SPI's MSB format */
...
/*/ SWD Sequence to Read Register 0 (IDCODE), prepadded with 2 null bits bits to fill up 6 bytes.
 * Byte-aligned, will not cause overrun error. */
/*/ A transaction must be followed by another transaction or at least 8 idle cycles to ensure that
 * data is clocked through the AP. */
/*/ After clocking out the data parity bit, continue to clock the SW-DP serial interface until it
 * has clocked out at least 8 more clock rising edges, before stopping the clock. */
...
/*/ Byte-aligned, will not cause overrun error. */
static const uint8_t swd_write_abort[]   = { 0x00, 0x81, 0xd3, 0x03, 0x00, 0x00, 0x00, 0x00 };	/*  With
												 * null
												 * byte
												 * (8
												 * cycles
...
static const struct command_registration bcm2835spi_command_handlers[] = {
	...
	{
		.name = "bcm2835spi_swdio_dir_num",
		.handler = &bcm2835spi_handle_swd_dir_gpionum_swdio,
		.mode = COMMAND_CONFIG,
		.help =
...
/*/ Transmit or receive bit_cnt number of bits from/into buf (LSB format) starting at the bit
 * offset. */
...
void spi_exchange(bool target_to_host, uint8_t buf[], unsigned int offset, unsigned int bit_cnt)
{
	...
	if (byte_cnt >= MAX_SPI_SIZE) {
		LOG_DEBUG("bit_cnt=%d ", bit_cnt); perror("spi_exchange: overflow"); return;
	}

	/*  Handle delay operation. */
	if (!buf) {
		/*  bitbang_swd_run_queue() calls bitbang_exchange() with buf=NULL and bit_cnt=8 for
		 * delay. */
		/*  bitbang_swd_write_reg() calls bitbang_exchange() with buf=NULL and bit_cnt=255
		 * for delay. */
	...
	/*  if (!buf) { LOG_DEBUG("offset=%d, bit_cnt=%d, ", offset, bit_cnt); perror("spi_exchange:
	 * null buffer"); return; } */

	/*  If target_to_host is true, receive from target to host. Else transmit from host to
	 * target. */
	if (target_to_host) {
...
static void spi_exchange_transmit(uint8_t buf[], unsigned int offset, unsigned int bit_cnt)
{
	...
	/*  This happens right after SWD Write Ack (5 bits), which doesn't receive SPI bytes. We
	 * compensate the SWD Write Ack before SWD Write Data. */
	if (offset == 5 && bit_cnt == 33) {
		/*  SWD Write Ack (5 bits) + SWD Write Data (33 bits) = 38 bits. Then pad later by 2
		 * bits to 40 bits to align by byte. */
		offset = 0;
		bit_cnt = 38;
	}
	/*  Otherwise we must be transmitting the SWD Command Header, which is 8 bits and
	 * byte-aligned: */
	...
	/*  Pad with null bits until the whole byte is populated.  Should be 2 bits for SWD Write
	 * Command. */
	int i = 0;
	...
	if (bit_cnt == 38) {	/*  SWD Write Command */
		/*  Add 8 clock cycles before stopping the clock.  A transaction must be followed by
		 * another transaction or at least 8 idle cycles to ensure that data is clocked
		 * through the AP. */
		/*  After clocking out the data parity bit, continue to clock the SW-DP serial
		 * interface until it has clocked out at least 8 more clock rising edges, before
		 * stopping the clock. */
...
/*/ SWD Read Data request is not byte-aligned, so this will always cause overrun error. We clear the
 * error in spi_transmit_resync(). */
static void spi_exchange_receive(uint8_t buf[], unsigned int offset, unsigned int bit_cnt)
{
	/*  Handle SWD Write Ack, which is 5 bits and not byte-aligned: */
	/*  ** trgt -> host offset 0 bits  5: 13 */
	/*  We always force return OK (0x13) without actually receiving SPI bytes. We compensate the
	 * 5 bits during SWD Write Data later (33 bits). */
	if (offset == 0 && bit_cnt == 5) {
		/*  LOG_DEBUG("write ack force OK\n"); */
		buf[0] = (buf[0] & 0b11100000) | 0x13;	/*  Force lower 5 bits to be 0x13 */
		return;
	}
	/*  Otherwise we must be receiving SWD Read Data, which is 38 bits and not byte-aligned. We
	 * will resync by transmitting JTAG-To-SWD below. */
	...
	/*  ** trgt -> host offset 0 bits 38: 73 47 01 ba a2 */
	/*  Since the target is in garbled state, we will resync by transmitting JTAG-To-SWD and
	 * Read IDCODE. */
...
static void spi_transmit_resync(int fd)
{
	...
	/*  Transmit JTAG-to-SWD sequence. Need to transmit every time because the SWD read/write
	 * command has extra 2 undefined bits that will confuse the target. */
	spi_transmit(fd, swd_seq_jtag_to_swd, swd_seq_jtag_to_swd_len / 8);

	/*  Transmit command to read Register 0 (IDCODE).  This is mandatory after JTAG-to-SWD
	 * sequence, according to SWD protocol.  We prepad with 2 null bits so that the next command
	 * will be byte-aligned. */
	spi_transmit(fd, swd_read_idcode_prepadded, swd_read_idcode_prepadded_len / 8);

	/*  Transmit command to write Register 0 (ABORT) and clear all sticky flags.  Error flags
	 * must be cleared before sending next transaction to target. */
	/*  We expect overrun errors because SWD Read requests are not byte-aligned. So we clear the
	 * error here. */
...
static void spi_transmit(int fd, const uint8_t *buf, unsigned int len)
{
    bcm2835_swdio_drive(true);
	/*  Reverse LSB to MSB for LSB buf into MSB buffer. */
	if (len >= MAX_SPI_SIZE) {
		LOG_DEBUG("len=%d ", len); perror("spi_transmit overflow"); return;
	}
...
static void spi_receive(int fd, uint8_t *buf, unsigned int len)
{
    bcm2835_swdio_drive(false);
	/*  Receive the MSB buffer from SPI device. */
#ifdef LOG_SPI
	LOG_DEBUG("spi_receive: len=%d\n  ", len);
#endif  //  LOG_SPI
	if (len >= MAX_SPI_SIZE) {
		LOG_DEBUG("len=%d ", len); perror("spi_receive overflow"); return;
	}
	struct spi_ioc_transfer tr = {
...
static void push_lsb_buf(int next_bit)
{
	...
	if (byte_cnt >= MAX_SPI_SIZE) {
		perror("push_lsb_buf: overflow"); return;
	}
...
static int pop_lsb_buf(void)
{
	unsigned int byte_cnt = (lsb_buf_bit_index + 7) / 8;	/*  Round up to next byte count. */
	if (byte_cnt >= MAX_SPI_SIZE) {
		perror("pop_lsb_buf: overflow"); return -1;
	}
...
static void bcm2835_swdio_drive(bool is_output)
{
	LOG_DEBUG("set drive to %s", is_output ? "out" : "in");
...
COMMAND_HANDLER(bcm2835spi_handle_speed)
{
    ...
    LOG_INFO("BCM2835 SPI: changing speed to -> %d kHz", speed_khz);
...
static int bcm2835spi_quit(void)
{
	/*  Close SPI device. */
	if (spi_fd < 0)
		return ERROR_OK;		/*  Terminate only once */
	close(spi_fd);
	spi_fd = -1;
	return ERROR_OK;
}
